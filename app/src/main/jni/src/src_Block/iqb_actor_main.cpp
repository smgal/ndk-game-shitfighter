...
{
__android_log_print(ANDROID_LOG_DEBUG, "SMGAL", "s_Initialize() #1");
#if (TARGET_DEVICE == TARGET_BADA)
avej_lite::util::SetResourcePath("res_Block");
#elif (TARGET_DEVICE == TARGET_ANDROID)
avej_lite::util::SetResourcePath("res_Block");
#endif
__android_log_print(ANDROID_LOG_DEBUG, "SMGAL", "s_Initialize() #2");

//	p_system = new CSystem(STATE_TITLE);
	p_system = new CSystem(STATE_GAME_PLAY);
__android_log_print(ANDROID_LOG_DEBUG, "SMGAL", "s_Initialize() #3");
	return (p_system != 0);
}
...
{
	__android_log_print(ANDROID_LOG_DEBUG, "SMGAL", "s_Process()");
	return p_system->Process();
}
