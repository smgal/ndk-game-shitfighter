...
void CInputDevice::UpdateInputState()
{
	...
	// in case of [one key, multi input]
...
namespace target
{
	void setTouchPos(int id, int x, int y)
	{
		...
		s_multi_touch_info_array[index].x = x;
...
void CInputDevice::UpdateInputState()
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		int y = s_multi_touch_info_array[i].y;
	...
	// for debugging
	{
		...
		if (s[0])
			dlog_print(DLOG_ERROR, "SMGAL", "pressed: %s", s);
	}
/*
...
namespace target
{
	...
	{
		s_fn_touch_region_callback = fn_callback;
	}
}
