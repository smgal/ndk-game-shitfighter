...
#elif (TARGET_DEVICE == TARGET_ANDROID)
...
namespace avej_lite
{
	namespace util
	{
		string s_app_path1("assets/");
		string s_app_path2;

		void SetAppPath(const string& path)
		{
			__android_log_print(ANDROID_LOG_DEBUG, "SMGAL", "App Path (%s)", path);
			//??
			// s_app_path1 = path;
		}
		...
		void SetResourcePath(const string& path)
		{
			s_app_path2 = path;
			s_app_path2 += "/";
			__android_log_print(ANDROID_LOG_DEBUG, "SMGAL", "App Path (%s)", s_app_path2);
		}
			...
			: m_p_impl(new TImpl)
		{
			...
			if (m_p_impl->m_p_buffer)
			{
				char sz_log[256];
				sprintf(sz_log, "[SMGAL] %s found!", (const char*)file_name);
				__android_log_print(ANDROID_LOG_DEBUG, "SMGAL", "%s", sz_log);
			}
			else
			{
				char sz_log[256];
		...
		bool  CFileReadStream::IsValidPos(void) const
		{
			...
			return (m_p_impl->m_offset >= 0 && m_p_impl->m_offset < m_p_impl->m_buffer_length);
		}
	}
}
